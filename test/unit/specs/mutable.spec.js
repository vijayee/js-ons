...
describe('Test Mutable creation', function () {
  ...
  it('Test Mutable assignment', function () {
    ...
    expect(mutable.contents).to.equal(record.key)
  })
  it('Test Mutable Reassignment', function () {
    let mutable2 = new Mutable(mutable.value)
    expect(mutable2.contents).to.equal(record.key)
    let url2 = OffUrl.parse('http://localhost:23402/offsystem/v3/offsystem/directory/7852/6LmAiRhsePn7EbR5umkKpRpqPHKgAz8dPDDKaYiBgk3T/3gRXxNxR8tW8xCLVXabRsfqfUfHbQVkZkNnbDJBLNGB9/html5up-prologue.ofd#')
